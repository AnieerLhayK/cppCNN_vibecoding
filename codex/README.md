...
开发阶段默认使用 `datasets/GTSRB_subset/`：10 类、每类 500 张训练图，共 5,000 张训练图。程序按数字目录排序，将原始类别映射为连续的 `0..N-1` 标签。传入完整数据根目录和 `43` 即可切换全部类别。完整下载链接、SHA-256、目录和子集映射见 [docs/dataset_guide.md](docs/dataset_guide.md)。
...
当前仍是搭建阶段，按照要求没有提前训练或伪造模型。正式模型训练完成后，可一条命令重新打包：
