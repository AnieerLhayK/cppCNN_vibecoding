...
- `gui/AppController`：资源发现、模型状态、图片状态和后台推理。
...
4. 演示图区：五张 GTSRB 图片，点击后自动推理。
5. 状态栏：模型路径、运行状态和可读错误。

`AppController::predict()` 使用 `QtConcurrent::run`。工作线程持有网络共享指针、输入张量副本和标签副本，完成后由 `QFutureWatcher` 在 GUI 线程更新属性，避免直接从工作线程访问 QML。
...
1. `applicationDir/models/gtsrb_subset10.bin`
2. `currentDir/codex/models/gtsrb_subset10.bin`
3. `currentDir/models/gtsrb_subset10.bin`
...
CMake 使用 `qt_generate_deploy_qml_app_script` 调用 Qt 官方部署工具。打包脚本将 Qt DLL、QML 模块、插件、模型、标签和演示图汇总到 `Release/`，并生成根目录 `qt.conf`。
