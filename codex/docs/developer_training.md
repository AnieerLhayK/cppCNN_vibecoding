...
codex/models/gtsrb_semantic10.bin
codex/models/gtsrb_semantic10.labels.txt
...
新模型训练完成后，可以在 GUI 的 Settings/模型选择入口手动加载，也可以在确认质量后修改默认模型配置并生成新的 Release。GUI 会优先读取与模型同名的 `.labels.txt`，例如 `gtsrb_semantic10.labels.txt`，避免不同类别映射之间发生标签错位。不要直接用未经评估的新模型替换稳定模型。

## 当前保留模型

| 模型 | 类别策略 | 训练图 | 测试图 | Top-1 |
| --- | --- | ---: | ---: | ---: |
| `gtsrb_subset10.bin` | 7 个限速类别的原开发基线 | 10,000 | 5,670 | 89.63% |
| `gtsrb_semantic10.bin` | 10 个语义分散类别 | 5,000 | 4,710 | 92.63% |
